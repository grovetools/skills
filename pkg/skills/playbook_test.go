...
`
	if err := os.WriteFile(filepath.Join(dir, "playbook.toml"), []byte(manifest), 0o644); err != nil { //nolint:gosec // G306: test
		t.Fatal(err)
	}

	// One skill
	skillDir := filepath.Join(dir, "skills", "hello")
	if err := os.MkdirAll(skillDir, 0o755); err != nil { //nolint:gosec // G301: test
		t.Fatal(err)
	}
	skillMd := `---
name: hello
...
Hello.
`
	if err := os.WriteFile(filepath.Join(skillDir, "SKILL.md"), []byte(skillMd), 0o644); err != nil { //nolint:gosec // G306: test
		t.Fatal(err)
	}

	// One prompt with purpose comment
	if err := os.MkdirAll(filepath.Join(dir, "prompts"), 0o755); err != nil { //nolint:gosec // G301: test
		t.Fatal(err)
	}
	prompt := "<!-- purpose: Bootstrap a chat with strict rules -->\n\nSome body text.\n"
	if err := os.WriteFile(filepath.Join(dir, "prompts", "bootstrap.md"), []byte(prompt), 0o644); err != nil { //nolint:gosec // G306: test
		t.Fatal(err)
	}

	// One prompt without purpose comment — should fall back to first paragraph
	fallback := "The first real line of the prompt.\n\nSecond paragraph.\n"
	if err := os.WriteFile(filepath.Join(dir, "prompts", "fallback.md"), []byte(fallback), 0o644); err != nil { //nolint:gosec // G306: test
		t.Fatal(err)
	}

	// One recipe
	if err := os.MkdirAll(filepath.Join(dir, "recipes"), 0o755); err != nil { //nolint:gosec // G301: test
		t.Fatal(err)
	}
	recipe := "---\ndescription: Standard test recipe\n---\n\nbody\n"
	if err := os.WriteFile(filepath.Join(dir, "recipes", "feature.md"), []byte(recipe), 0o644); err != nil { //nolint:gosec // G306: test
		t.Fatal(err)
	}
...
func TestLoadPlaybookViaSearchPath(t *testing.T) {
	...
	if err := os.MkdirAll(pbDir, 0o755); err != nil { //nolint:gosec // G301: test
		t.Fatal(err)
	}
	manifest := `name = "tiny"
description = "Tiny"
version = "0.0.1"
`
	if err := os.WriteFile(filepath.Join(pbDir, "playbook.toml"), []byte(manifest), 0o644); err != nil { //nolint:gosec // G306: test
		t.Fatal(err)
	}
...
func TestLoadPlaybook_FromRegisteredWorkDir(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(pbDir, "skills"), 0o755); err != nil { //nolint:gosec // G301: test
		t.Fatal(err)
	}
